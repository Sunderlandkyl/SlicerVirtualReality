...
- [`VirtualReality/Resources/Bindings/vtk_openxr_binding_hand_interaction.json`][vtk_openxr_binding_hand_interaction_url] binds action names to the `/interaction_profiles/ext/hand_interaction_ext` profile, exhaustively covering every component the profile defines (`grip`/`aim`/`pinch_ext`/`poke_ext` poses; `pinch_ext`/`grasp_ext`/`aim_activate_ext` values, click thresholds, and `ready_ext` flags), the same way `vtk_openxr_binding_oculus_touch_controller.json` covers every physical control on the Touch controller — see the `ControllerEvents` doc comment in `vtkVirtualRealityViewOpenXRInteractorStyle.h` for the full list. Only two of these are translated into default behavior: `left_pinch_click`/`right_pinch_click` (grab, thumb tip to index fingertip) drive `PositionProp3DEvent` exactly like `left_grip_click`/`right_grip_click` do for controllers; `left_poke_pose`/`right_poke_pose` (fingertip-pointing direction, fired every frame that pose is tracked) alone drives point-to-fly, with no separate click/grasp gesture gating it — this one *does* require interactor-style code, since it works by reacting to the pose event's mere presence/absence each frame rather than to a Press/Release edge (see the `LeftPokePoseEvent` doc comment in `vtkVirtualRealityViewOpenXRInteractorStyle.h` for the full mechanism, including why its `Action` field can't safely be read). Every other exposed gesture (e.g. `aim_activate_ext`, which this Meta Quest Link runtime version was found not to implement at all, or `grasp_ext`, an earlier iteration's fly trigger that turned out to be unreliable on this runtime and was dropped in favor of the poke-pose-alone approach) is registered and independently observable like any other `ControllerEvents` value, but not wired into a default translation — see "Low-level interception of events" below to hook up an alternative gesture from Python. All of this requires `XR_EXT_hand_interaction` to be requested at `xrCreateInstance` time, added in `vtkOpenXRManager::SelectExtensions()`.

Both binding files (plus the shared actions manifest) are deployed under this module's own share directory (see `vtkSlicerVirtualRealityLogic::ComputeActionManifestPath()`), not vtkRenderingOpenXR's. Refer to the [Reserved Paths](https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html#semantic-path-reserved) and [Interaction Profile Paths](https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html#semantic-path-interaction-profiles) sections of the OpenXR spec for background on the path format.
...
[vtk_openxr_binding_hand_interaction_url]: https://github.com/KitwareMedical/SlicerVirtualReality/blob/master/VirtualReality/Resources/Bindings/vtk_openxr_binding_hand_interaction.json
